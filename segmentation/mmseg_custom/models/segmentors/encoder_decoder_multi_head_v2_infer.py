...
class EncoderDecoderMultiHeadV2Infer(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(EncoderDecoderMultiHeadV2Infer, self).__init__(init_cfg)
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg, aux_head_num):
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        loss_decode, aux_head_input = self.decode_head.forward_train(x, img_metas,
                                                                    gt_semantic_seg,
                                                                    self.train_cfg, 
                                                                    aux_head_num)

        losses.update(add_prefix(loss_decode, 'decode'))
        return losses, aux_head_input
    ...
    def _auxiliary_head_forward_train(self, x, aux_head_input, img_metas, gt_semantic_seg, **kwargs):
        ...
        if len(self.auxiliary_head) == len(aux_gt_list) + 1:
            ...
            if isinstance(self.auxiliary_head, nn.ModuleList):
                for idx, aux_head in enumerate(self.auxiliary_head):
                    if idx == 0:
                        # first aux head is for target task
                        loss_aux = aux_head.forward_train(x, img_metas,
                                                        gt_semantic_seg,
                                                        self.train_cfg)
                    else:
                        # other aux heads if for aux tasks
                        loss_aux = aux_head.forward_train([aux_head_input[idx-1]], img_metas, 
        ...
        elif len(self.auxiliary_head) == len(aux_gt_list):
            # all the aux heads are for aux tasks
            if isinstance(self.auxiliary_head, nn.ModuleList):
                for idx, aux_head in enumerate(self.auxiliary_head):
                        loss_aux = aux_head.forward_train([aux_head_input[idx]], img_metas, 
                                                        kwargs[aux_gt_list[idx]],
                                                        self.train_cfg)
                losses.update(add_prefix(loss_aux, f'aux_{idx}'))
            else:
                loss_aux = self.auxiliary_head.forward_train(
                    [aux_head_input[0]], img_metas, kwargs[aux_gt_list[0]], self.train_cfg)
                losses.update(add_prefix(loss_aux, 'aux'))

        else:
            raise AttributeError("Num of aux head and num of aux_gt do not match!")
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, **kwargs):
        ...
        aux_head_num = len(img_metas[0]["seg_fields"][1:])

        loss_decode, aux_head_input = self._decode_head_forward_train(x, img_metas,
                                                                    gt_semantic_seg, aux_head_num)
        losses.update(loss_decode)

        if self.with_auxiliary_head:
            loss_aux = self._auxiliary_head_forward_train(
                x, aux_head_input, img_metas, gt_semantic_seg, **kwargs)
