...
class SAMAdapterVPTSPMFuseHier(SAMViTVPT):
    def __init__(
        self,
        pretrain_size=1024,
        conv_inplane=64, 
        ...
        if self.spm_fuse_mode == "concat":
            self.fuse1 = nn.Linear(embed_dim * 2, embed_dim)
            self.fuse2 = nn.Linear(embed_dim * 2, embed_dim)
            self.fuse3 = nn.Linear(embed_dim * 2, embed_dim)
            self.fuse4 = nn.Linear(embed_dim * 2, embed_dim)
        ...
        self.fuse1.apply(self._init_weights)
        self.fuse2.apply(self._init_weights)
        self.fuse3.apply(self._init_weights)
        self.fuse4.apply(self._init_weights)
    ...
    def forward(self, x, aux_gt):
        ...
        if self.spm_fuse_mode == "concat":
            c1 = self.fuse1(torch.cat([r1, t1], dim=-1)).transpose(1, 2).view(bs, dim, H, W)
            c2 = self.fuse2(torch.cat([r2, t2], dim=-1))
            c3 = self.fuse3(torch.cat([r3, t3], dim=-1))
            c4 = self.fuse4(torch.cat([r4, t4], dim=-1))

        elif self.spm_fuse_mode == "add":
            c1 = self.fuse1(r1 + t1).transpose(1, 2).view(bs, dim, H, W)
            c2 = self.fuse2(r2 + t2)
            c3 = self.fuse3(r3 + t3)
            c4 = self.fuse4(r4 + t4)
