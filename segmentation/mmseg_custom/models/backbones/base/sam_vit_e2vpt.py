...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt_emb: torch.Tensor, kv_prompt_emb: torch.Tensor) -> torch.Tensor:
        B, H, W, C = x.shape
        prompt_num, _ = prompt_emb.shape
        head_num, kv_prompt_num, _ = kv_prompt_emb.shape

        shortcut = x
        x = self.norm1(x)
        prompt_emb = prompt_emb.expand(B, -1, -1)
        kv_prompt_emb = kv_prompt_emb.expand(B, -1, -1, -1)
        # Window partition
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt_emb: torch.Tensor, kv_prompt_emb: torch.Tensor) -> torch.Tensor:
        ...
        # k, v with shape (B * nHead, kv_prompt_num + prompt_num + H * W, C/nHead)
        k = torch.cat((kv_prompt_emb, k), dim=1)
        v = torch.cat((kv_prompt_emb, v), dim=1)

        # attn with shape (B * nHead, prompt_num + H * W, kv_prompt_num + prompt_num + H * W)
        attn = (q * self.scale) @ k.transpose(-2, -1)
        # attn_crop with shape (B * nHead, H * W, H * W)
        attn_crop = attn[:, prompt_num:, kv_prompt_num + prompt_num:]
        q_crop = q[:, prompt_num:, :]

        if self.use_rel_pos:
            attn_crop = add_decomposed_rel_pos(attn_crop, q_crop, self.rel_pos_h, self.rel_pos_w, (H, W), (H, W))

        attn[:, prompt_num:, kv_prompt_num + prompt_num:] = attn_crop
        attn = attn.softmax(dim=-1)
        # x with shape (B * nHead, prompt_num + H * W, C/nHead)
        x = attn @ v
