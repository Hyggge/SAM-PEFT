...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.adaptformer = Adapter(
            d_model=dim,
            bottleneck=adaptformer_bottleneck,
            dropout=adaptformer_dropout,
            init_option=adaptformer_init_option,
            adapter_scalar=adaptformer_scalar,
            adapter_layernorm_option=adaptformer_layernorm_option
        )
        
        self.adaptformer_option = adaptformer_option
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.adaptformer_option == 'parallel':
            adapt_x = self.adaptformer(x, add_residual=False)
            x = self.mlp(self.norm2(x)) + adapt_x
        else:
            x = self.adaptformer(self.mlp(self.norm2(x)))
