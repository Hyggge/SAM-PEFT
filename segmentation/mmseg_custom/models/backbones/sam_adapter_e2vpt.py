...
from .base.sam_vit_e2vpt import SAMViTE2VPT
...
class SAMAdapterE2VPT(SAMViTE2VPT):
    def __init__(
        self,
        pretrain_size=1024,
        conv_inplane=64, 
        ...
        self.kv_prompt_dropout = nn.Dropout(kv_prompt_dropout)
        ...
        if kv_prompt_project > -1:
            kv_prompt_dim = kv_prompt_project
            self.kv_prompt_proj = nn.Linear(kv_prompt_dim, embed_dim // encoder_num_heads)
            nn.init.kaiming_normal_(
                self.kv_prompt_proj.weight,  a=0, mode='fan_out')
        else:
            kv_prompt_dim = embed_dim // encoder_num_heads
            self.kv_prompt_proj = nn.Identity()
        ...
        self.kv_prompt_embeddings = nn.Parameter(torch.zeros(encoder_depth, encoder_num_heads, kv_prompt_token_num, kv_prompt_dim))
        nn.init.uniform_(self.kv_prompt_embeddings, -val, val)
        ...
        for i, blk in enumerate(self.blocks):
            wrapped_blocks.append(
                WrappedBlock(
                    ...
                    self.kv_prompt_dropout(self.kv_prompt_proj(self.kv_prompt_embeddings[i]))
                )
            )
...
class WrappedBlock(nn.Module):
    def __init__(self, block, prompt_emb, kv_prompt_emb):
        super().__init__()
        self.block = block
        self.prompt_emb = prompt_emb
        self.kv_prompt_emb = kv_prompt_emb
    ...
    def forward(self, x, H, W):
        ...
        """
        bs, n, c = x.shape
        return self.block(x.view(bs, H, W, c), self.prompt_emb, self.kv_prompt_emb).view(bs, n, c)
