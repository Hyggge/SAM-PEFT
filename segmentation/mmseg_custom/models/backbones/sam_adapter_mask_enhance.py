...
class SAMAdapterMaskEnhance(SAMViT):
    def __init__(
        self,
        pretrain_size=1024,
        conv_inplane=64, 
        ...
        self.proj = nn.Linear(embed_dim, embed_dim) if use_proj else nn.Identity()
    ...
    def forward(self, x, aux_gt):
        ...
        mask = (mask != 0).repeat(1, self.embed_dim, 1, 1)
        c1 = c1 + self.proj(c1.masked_fill(mask == 0, 0))
