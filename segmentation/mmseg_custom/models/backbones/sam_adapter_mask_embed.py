...
class SAMAdapterMaskEmbed(SAMViT):
    def __init__(
        self,
        pretrain_size=1024,
        conv_inplane=64, 
        ...
        # Mask Encoder parameter 
        mask_in_chans=16,
        only_binary_mask=True,
        mask_embed_dim=256
    ) :
        super().__init__(
            depth=encoder_depth,
        ...
        self.only_binary_mask = only_binary_mask
        self.mask_downscaling = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(mask_in_chans, mask_embed_dim, kernel_size=1),
        )
        self.mask_neck = nn.Sequential(
            LayerNorm2d(mask_embed_dim),
            nn.GELU(),
            nn.Conv2d(mask_embed_dim, embed_dim, kernel_size=1),
        )

        self.up.apply(self._init_weights)
        self.spm.apply(self._init_weights)
        self.interactions.apply(self._init_weights)
        self.mask_neck.apply(self._init_weights)
        self.apply(self._init_deform_weights)
        self._load_mask_encoder_weight(pretrained)
        normal_(self.level_embed)

    def _load_mask_encoder_weight(self, pretrained):
        if isinstance(pretrained, str):
            state_dict = torch.load(pretrained, map_location="cpu")
            new_dict = {}
            for key in state_dict.keys():
                if key.startswith("prompt_encoder.mask_downscaling"):
                    new_dict[key[15:]] = state_dict[key]
    ...
    def forward(self, x, aux_gt):
        ...
        if self.add_vit_feature:
            x1, x2, x3, x4 = outs
            x1 = x1 + mask_embedding
            x2 = x2 + mask_embedding
            x3 = x3 + mask_embedding
            x4 = x4 + mask_embedding
